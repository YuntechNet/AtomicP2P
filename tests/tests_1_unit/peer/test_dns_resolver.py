...
def test_forward(dns_resolver):
    ...
    assert l == []
    l = dns_resolver.forward('peer_1.switch.atomic_p2p.unittest.org')
...
def test_reverse(dns_resolver):
    ...
    assert l[0] == 'peer_1.switch.atomic_p2p.unittest.org.'


def test_srv(dns_resolver):
    l = dns_resolver.srv('1234')
    assert l == (0, 0, -1, None)
    l = dns_resolver.srv('peer_1.switch.atomic_p2p.unittest.org')
    assert l == ('0', '0', '8000', 'peer_1.switch.atomic_p2p.unittest.org.')