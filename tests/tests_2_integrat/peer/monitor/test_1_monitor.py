...
def test_two_dogs(core1, edge1):
    ...
    max_no_res_count = max(core1.monitor.max_no_response_count,
                           edge1.monitor.max_no_response_count) + 1
...
def test_three_dogs(core1, edge1, edge2):
    ...
    assert edge1.get_peer_info_by_host(host=core1.server_info.host) is not None
    assert \
        edge1.get_peer_info_by_host(host=edge2.server_info.host) is not None

    assert edge2.get_peer_info_by_host(host=core1.server_info.host) is not None
    assert \
        edge2.get_peer_info_by_host(host=edge1.server_info.host) is not None

    max_loop_delay = max(core1.monitor.loopDelay,
                         edge1.monitor.loopDelay,
                         edge2.monitor.loopDelay)
    max_no_res_count = max(core1.monitor.max_no_response_count,
                           edge1.monitor.max_no_response_count,
                           edge2.monitor.max_no_response_count) + 1
